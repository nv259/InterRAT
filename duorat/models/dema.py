...
@registry.register("model", "InterRAT")
class InterRATModel(LMDuoRATModel):
    """
    InterRAT: A LM DuoRAT model with first layer is cloned multiple times
    """
    ...
    def compute_branch_loss(
        self, preproc_items: torch.List[RATPreprocItem], particle_idx: int, debug=False
    ) -> torch.Tensor:
        duo_rat_batch = self.items_to_duo_rat_batch(preproc_items)
        decoder_batch = duo_rat_batch.decoder_batch

        memory, output = self.forward_branch(
            batch=duo_rat_batch, particle_idx=particle_idx
        )
    ...
    def forward_branch(
        self, batch: DuoRATBatch, particle_idx: int
    ...
    def _encode_branch(
        self, batch: DuoRATEncoderBatch, particle_idx: int
    ) -> torch.Tensor:
